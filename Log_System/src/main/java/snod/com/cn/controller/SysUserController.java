...
import snod.com.cn.entity.vo.UpdatePasswordDto;
...
//				.eq(SysUser::getShopId, SecurityUtils.getSecurityUser().getShopId())
//				.like(StrUtil.isNotBlank(username), SysUser::getUsername, username));

 		IPage<SysUser> sysUserPage=sysUserService.queryList(page,username);
...
//	@SysLog("修改密码")
	...
	public ResponseEntity<String> password(@RequestBody @Valid UpdatePasswordDto param){
		Long userId = SecurityUtils.getSecurityUser().getUserId();
...
//		更新密码
	...
	public ResponseEntity<SysUser> info(@PathVariable("userId") Long userId){
		...
		user.setUserId(null);
		if (!Objects.equals(user.getShopId(), SecurityUtils.getSecurityUser().getShopId())) {
			throw new UnauthorizedExceptionBase("没有权限获取该用户信息");
		}
		//获取用户所属的角色列表
...
//	@SysLog("保存用户")
	...
	public ResponseEntity<String> save(@Valid @RequestBody SysUser user){
		...
		SysUser dbUser = sysUserService.getByUserName(username);
		if (dbUser!=null) {
			return ResponseEntity.badRequest().body("该用户已存在");
		}
		user.setShopId(SecurityUtils.getSecurityUser().getShopId());
...
//	@SysLog("修改用户")
	...
	public ResponseEntity<String> update(@Valid @RequestBody SysUser user){
		...
		if (!Objects.equals(dbUser.getShopId(), SecurityUtils.getSecurityUser().getShopId())) {
			throw new UnauthorizedExceptionBase("没有权限修改该用户信息");
		}
...
//	@SysLog("删除用户")
	...
	public ResponseEntity<String> delete(@RequestBody Long[] userIds){
		...
		if(ArrayUtil.contains(userIds, SecurityUtils.getSecurityUser().getUserId())){
			return ResponseEntity.badRequest().body("当前用户不能删除");
		}
		sysUserService.deleteBatch(userIds,SecurityUtils.getSecurityUser().getShopId());
		// 删除缓存
